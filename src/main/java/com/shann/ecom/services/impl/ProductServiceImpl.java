...
public class ProductServiceImpl implements ProductService {
    ...
    public ProductServiceImpl(GoogleMapsAdapters googleMapsAdapters, ProductRepository productRepository, AddressRepository addressRepository, DeliveryHubRepository deliveryHubRepository){
    ...
    public Date estimateDeliveryDate(int productId, int addressId)
            throws ProductNotFoundException, AddressNotFoundException {
                ...
                var product = productRepository.findById(productId).orElseThrow(()-> new ProductNotFoundException("Product with: "+productId+" not found"));
                
                var seller = product.getSeller();

                var address = addressRepository.findById(addressId).orElseThrow(()-> new AddressNotFoundException("There is no address with: "+addressId));
                var deliveryHub  = deliveryHubRepository.findByZipCode(address.getZipCode()).get();
                var timeToReachDeliveryHub = mapsAdapters.getEstimatedTime(seller.getAddress().getLatitude(), seller.getAddress().getLongitude(), deliveryHub.getAddress().getLatitude(), deliveryHub.getAddress().getLongitude());

                var timeToReachUser = mapsAdapters.getEstimatedTime(deliveryHub.getAddress().getLatitude(), deliveryHub.getAddress().getLongitude(), address.getLatitude(), address.getLongitude());
